...
First, in order to get a handle on the magnitude of the missing values problem, we compute the number of cases with NAs in them (`r sum(is.na(activity[,1]))`), and the proportion of the total that represents (`r sum(is.na(activity[,1]))/length(activity[,1])`).
